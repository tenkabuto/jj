...
fn merge_commits(
    ...
    let target_tree = repo.find_commit(target).unwrap().tree().unwrap();

    let labels = gix::merge::blob::builtin_driver::text::Labels {
        ancestor: Some(b"ancestor".into()),
        current: Some(b"base".into()),
        other: Some(b"target".into()),
    };
    let options = repo.tree_merge_options().unwrap();
    ...
    assert!(!outcome.failed_on_first_unresolved_conflict);
    let tree_id = outcome.tree.write().unwrap().detach();
...
fn test_git_fetch_from_remote_named_git(subprocess: bool) {
    ...
    git::add_remote(&git_repo, "git", "../git");
...
fn test_git_fetch_from_remote_with_slashes(subprocess: bool) {
    ...
    let git_repo = git::init(&repo_path);
    git::add_remote(&git_repo, "slash/origin", "../source");
...
fn test_git_fetch_preserve_commits_across_repos(subprocess: bool) {
    ...
    let fork_remote = git::add_remote(&upstream_repo, "fork", fork_path.to_str().unwrap());
    fork_remote
    ...
    insta::allow_duplicates! {
    insta::assert_snapshot!(get_bookmark_output(&test_env, &repo_path), @r"
    upstream: yvyoprst 5ab964fc merge
